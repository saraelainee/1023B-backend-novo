...
class CarrinhoController {
    ...
    async adicionarItem(req:Request, res:Response) {
        ...
        res.status(200).json({ message: "Eba, item adicionado ao carrinho com sucesso!" });          
    } 
    ...
    await db.collection("carrinhos").updateOne(
     ...
     { pull: { itens: { produtoId: produtoId } } }
        ...
        res.status(200).json({ message: "Eba, item removido do carrinho com sucesso!" });
    }
    ...
    async remover(req: Request, res: Response) {
        ...
        res.status(200).json({ message: "Eba, carrinho removido com sucesso" });
    }
}
